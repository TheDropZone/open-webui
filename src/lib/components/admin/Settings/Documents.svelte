<script lang="ts">
	...
	onMount(async () => {
		...
		// Convert external loader JSON fields to strings for textarea display
		// Show empty string instead of {} to display placeholder
		const paramsObj = config.EXTERNAL_DOCUMENT_LOADER_PARAMS ?? {};
		config.EXTERNAL_DOCUMENT_LOADER_PARAMS = Object.keys(paramsObj).length > 0
			? JSON.stringify(paramsObj, null, 2)
			: '';

		const queryParamsObj = config.EXTERNAL_DOCUMENT_LOADER_QUERY_PARAMS ?? {};
		config.EXTERNAL_DOCUMENT_LOADER_QUERY_PARAMS = Object.keys(queryParamsObj).length > 0
			? JSON.stringify(queryParamsObj, null, 2)
			: '';

		const headersObj = config.EXTERNAL_DOCUMENT_LOADER_HEADERS ?? {};
		config.EXTERNAL_DOCUMENT_LOADER_HEADERS = Object.keys(headersObj).length > 0
